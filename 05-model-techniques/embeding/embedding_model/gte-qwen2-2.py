...
def last_token_pool(last_hidden_states: Tensor,
    ...
    # 检查是否为左侧填充（即所有序列最后一个位置都有效）【如果最后一列的求和和行数相同，说明填充是左边，因为最后一列也就是右边都是1】
    ...
    if left_padding:
        ...
        # 返回每个序列最后一个有效token的隐藏状态（每一批都取一个当前的sequence_length）
...
# 加载模型 用AutoModel而不是AutoModelForCausalLM 是这里只加载编码器，不用来生成只用来输出原本的句子
...
# .T：转置操作
# * 100：将相似度分数缩放到0-100的范围 【四段话，前两个和后两个进行相似度比较】
